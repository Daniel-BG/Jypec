...
public class PrincipalComponentAnalysis implements DimensionalityReduction {
	 ...
	 *
	 * @param numComponents
	 *            Number of vectors it will use to describe the data. Typically
	 *            much smaller than the number of elements in the input vector.
	 */
	...
	public void train(HyperspectralImage source) {
		if (this.numComponents <= 0) {
			throw new IllegalStateException(
					"Please first set the number of components for this dimensionality reduction algorithm");
		}
	...
	public double getMinValue(HyperspectralImage img) {
		return -MathOperations.getMaximumDistance(img.getDataType().getMagnitudeAbsoluteRange(),
				img.getNumberOfBands());
	}

}

/*
 * Older PCA version. public void computeBasis2( int numComponents ) { if(
 * numComponents > this.sampleSize ) throw new
 * IllegalArgumentException("More components requested that the data's length."
 * ); if( sampleIndex != A.getNumRows() ) throw new
 * IllegalArgumentException("Not all the data has been added"); if(
 * numComponents > sampleIndex ) throw new
 * IllegalArgumentException("More data needed to compute the desired number of components"
 * );
 * 
 * this.numComponents = numComponents;
 * 
 * // compute the mean of all the samples for( int i = 0; i < A.getNumRows();
 * i++ ) { for( int j = 0; j < mean.length; j++ ) { mean[j] += A.get(i,j); } }
 * for( int j = 0; j < mean.length; j++ ) { mean[j] /= A.getNumRows(); }
 * 
 * // subtract the mean from the original data for( int i = 0; i <
 * A.getNumRows(); i++ ) { for( int j = 0; j < mean.length; j++ ) {
 * A.set(i,j,A.get(i,j)-mean[j]); } }
 * 
 * // Compute SVD and save time by not computing U
 * SingularValueDecomposition<DMatrixRMaj> svd =
 * DecompositionFactory_DDRM.svd(A.numRows, this.sampleSize, false, true,
 * false); if( !svd.decompose(A) ) throw new RuntimeException("SVD failed");
 * 
 * V_t = svd.getV(null,true); DMatrixRMaj W = svd.getW(null);
 * 
